...
- Retrain and commit `best_broadcast_model.pkl` after cloning (run `Views/main.ipynb` or training cells)
- Executive summary KPIs in `master_dashboard.py` use sample trend data; connect live aggregates from module outputs for production
- Explore hyperparameter tuning, rolling-window CV, and external calendars (holidays, competing TV events)
- Optional: deploy Streamlit app via Streamlit Cloud / container; serve `webapp/` as static landing page
...
streamlit run Ticket/sample.py
streamlit run Sponser/sponsorship_dashboard.py
streamlit run Merchandies/train_merch_models.py
```

### Train sponsorship model (CLI)

```bash
python Sponser/train_sponsorship_value.py Sponser/sponsorship_value_cricket.csv
```

### Viewership model artifact

Train via `Views/main.ipynb`, then ensure these files exist in `Views/`:

- `best_broadcast_model.pkl`
- `model_features.pkl`
...
| Item | Detail |
|------|--------|
| **Organization** | Binary Stitch Software LLP (internship / major project) |
| **Domain** | IPL / professional cricket market analytics |
| **Primary interface** | Streamlit (`master_dashboard.py`) |
| **License / usage** | Academic and internship project — refer to your organization for distribution terms |
